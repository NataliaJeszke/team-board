...
import { TASK_PRIORITIES, TASK_STATUSES, TaskStatus, TaskPriority } from '@core/models';

export const TASK_STATUS_OPTIONS: readonly FilterOption[] = TASK_STATUSES.map((status: TaskStatus) => ({
  label: `features.tasks.filters.status.${status}`,
  value: status,
}));

export const TASK_PRIORITY_OPTIONS: readonly FilterOption[] = TASK_PRIORITIES.map((priority: TaskPriority) => ({
